...
func main(){
	log.Println("Authentication webhook")
	...
	secret := flag.String("secret","mysecret","Secret for authentication")
	flag.Parse()
	log.Println("certificate:  ",*cert," private key : ",*key," protocol : ",*protocol," port : ",*port)
	r:= mux.NewRouter()	
	authentication := Authn{ Secret: *secret}
	r.HandleFunc("/",authentication.CheckAuthn).Methods("POST")
